...
# Default books
FAMOUS_BOOKS = [
    {"Title": "The Great Gatsby", "Author": "F. Scott Fitzgerald", "Year": 1925, "Genre": "Fiction", "Read": False, "PDF_Link": "https://www.planetebook.com/free-ebooks/the-great-gatsby.pdf"},
    {"Title": "1984", "Author": "George Orwell", "Year": 1949, "Genre": "Dystopian", "Read": False, "PDF_Link": "https://www.planetebook.com/free-ebooks/1984.pdf"},
    {"Title": "Pride and Prejudice", "Author": "Jane Austen", "Year": 1813, "Genre": "Romance", "Read": False, "PDF_Link": "https://www.planetebook.com/free-ebooks/pride-and-prejudice.pdf"}
]
...
if menu == "Add Book":
    ...
    for index, row in df.iterrows():
        read_status = "✅ Read" if row["Read"] else "❌ Unread"
        st.write(f"📖 **{row['Title']}** by *{row['Author']}* ({int(row['Year'])}) - {row['Genre']} - {read_status}")
        col1, col2 = st.columns(2)
        with col1:
            if pd.notna(row["PDF_Link"]) and row["PDF_Link"].strip() != "":
                st.markdown(f"[📖 Read Now]({row['PDF_Link']})")
        with col2:
            if st.button(f"❌ Remove", key=index):
